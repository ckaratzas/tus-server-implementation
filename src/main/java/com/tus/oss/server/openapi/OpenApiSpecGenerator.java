...
import static com.tus.oss.server.openapi.AnnotationMappers.decorateOperationFromAnnotation;

/**
 * @author ckaratza
 * Tries to interrogate vertx router and build an OpenAPI specification. Tries to interrogate handlers of route with
 * OpenApi Operation methods and cross-reference with route information.
 */
                        ...
                        Arrays.stream(delegate.getDeclaredMethods()).distinct().forEach(method -> {
                            ...
                            if (annotation != null) {
                                ...
                                if (matchedOperation != null)
                                    decorateOperationFromAnnotation(annotation, matchedOperation);
                            }
                        });
